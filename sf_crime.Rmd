...
crime.data$output = as.numeric(crime.data$Category == "LARCENY/THEFT")
...
fit = glm(output ~  ., data = tr_dat, family = binomial)
...
plot(density(predicted[te_dat$output == 1]), col="green", main="Double Density Plot")
lines(density(predicted[te_dat$output == 0]), col="red")

conf_matrix = table(predicted, te_dat$output)
...
for(i in 1:nrow(fit2$tables$Conditions)){

barplot(sort(fit2$tables$Conditions[,i]), horiz = T, las = 2, main = names(fit2$tables$Conditions[1,][i]) )
}

names(sort(fit2$tables$Conditions[1,]))
