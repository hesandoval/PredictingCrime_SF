...
par(mar=c(5,6,3,2))
#crime.data$Date = as.factor(dates)
...
sapply(crime_categories, function(c)
  barplot(table(crime.data[crime.data$Category == c,]$Hour), main = paste("Times when", c, "has been reported")))

san_fran = get_map(location="San Francisco", maptype = "roadmap", zoom = 12)

ggmap(san_fran) + geom_polygon(aes(x=long, y=lat, group=group), fill=NA, size=.5,color='black', data=polygon.data, alpha=1) + geom_tile(data = crime.data, aes(x = lng, y = lat, alpha = Frequency), fill = 'red')

sapply(crime_categories, function(c){
  san_fran = get_map(location="San Francisco", maptype = "roadmap", zoom = 12)
ggmap(san_fran) + geom_polygon(aes(x=long, y=lat, group=group), fill=NA, size=.5,color='black', data=polygon.data, alpha=1) + geom_tile(data = crime.data[crime.data$Category == c,], aes(x = lng, y = lat, alpha = Frequency), fill = 'red')
})
