...
te_data = data.split[[2]]

fit = glm(output ~ Events + Humidity + Conditions + TemperatureF + Tract + DayOfWeek + Hour, data = tr_data, family = binomial)


summary(fit)

predicted = predict(fit, newdata=te_data , type = "response")

plot(density(predicted[te_data$output == 0]), col="green", main="Double Density Plot")
lines(density(predicted[te_data$output == 1]), col="red")

conf_matrix = table(predicted, te_data$output)
...
>>>>>>> 006100f600238809c8d9703d74857887aff60d14
