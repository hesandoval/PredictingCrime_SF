...
# Reading Data
For our project we wanted to determine if there was a method to model crime In San Francisco. More specifically we wanted to see if it was possible to predict where a crime was going to occur given geospacial features, time, and weather conditions. Crime can be though of as an air bubble under a rug. It is easy to smooth out the area, but the bubble will often times travel to another side of the rug. With predictive analysis of crime it is possible to determine where where this bubble will appear before smoothing out the previous bump. Crime is a very important thing to predict because even a small reduction in it can lead to a huge increase in quality of life of all of the residents. Below we read in the data and load necesarry libraries and functions. 
...
#setwd("C:/Users/Matthew/Desktop/PredictingCrime_SF-master")
source("functions.r")
...
The U.S. Census separates cities into tracts, San Francisco has 197. We decided to add these tracts to our data as a feature. To do this we located a shape file containing polygons corrsponding to the census tracts of San Francisco. Then we checked which tract each crime was associated with and added that tract to the row for that crime.
...
#Mactch crime point to a tract and create a data frame.
...
#Combine tract data to crime data file
...
# Data Visualization
We wanted to examine the categories of crimes across a variety of fractors. Below you will find the distribution of crimes by day of week, month, and hour. The last group of graphs show heatmaps where crimes were committed in San francisco with tract overlays.
...
## Logistic Regression
The first model we built for our project was a logistic regression model. On this model we tried to see if we could determine the probability of a crime occurring. We added a column, output, to crime.data that was a binary indicator variable. We wanted to look specifically at robberies to simplify the model. In the output column, a value of one indicated the category roberry and every other row was given a 0. 
...
```
Our model did not do that great. We got a fairly low accuracy with the threshold we chose and both of the curves on the double density plot had relatively the same shape and size. We believe that the model didn't do so well because our use of the output column was not very approriate. We compared robberies to all other crimes, and it is very possible that those other crimes occured under the same circumstances as robberies in San Francisco. We wanted to move forward with this model, but we were unsure of how to create vectors for actual times when no robberies occured. 
...
## Naive Bayes to classify Categry
...
The final model we wanted to create was a model that classified crimes to a specific census tract. This model would, given a set of features, give a police officer from a given police district, the ability to patrol a tract that could potentially be more prone to the ctime they are looking for. 
